...
fn render_home<'a>() -> Paragraph<'a> {
    let span_raw = |text| Spans::from(vec![Span::raw(text)]);
    let span_style = |text: String, style| Spans::from(vec![Span::styled(text, style)]);
...
fn render_pets<'a>(pet_list_state: &ListState) -> (List<'a>, Table<'a>) {
    ...
    let pet_details = Table::new(vec![Row::new(
        ...
        .iter()
        .map(|field| Cell::from(Span::raw(field.clone())))
        .collect::<Vec<Cell>>(),
    )])
    .header(Row::new(
